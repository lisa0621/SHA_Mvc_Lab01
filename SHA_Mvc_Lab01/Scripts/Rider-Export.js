...
(function (windows) {
    ...
    function ExportData(exportFileName, selectedColumns) {
        ...
        $.ajax({
            type: 'post',
            url: Router.action('Rider', 'HasData'),
            dataType: 'json',
            cache: false,
            async: false,
                ...
                if (data.Msg) {
                    ...
                    if (HasData == 'False') {
                        ...
                        window.location = exportFileName.length == 0
                            ? Router.action('Rider', 'Export', { selectedColumns: selectedColumns })
                            : Router.action('Rider', 'Export', { fileName: exportFileName, selectedColumns: selectedColumns });
